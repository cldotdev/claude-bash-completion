...
- Auto-completion for CLI flags and their values (77 flags)
...
> Command, flag, and subcommand counts reflect Claude Code v2.1.258. The lists cover what a default account can actually run, including command aliases and entries that `claude --help` hides, such as `claude attach`. Commands that a feature flag leaves switched off are excluded, so completion never offers anything the CLI will refuse.
